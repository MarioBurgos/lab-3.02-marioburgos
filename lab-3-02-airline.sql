...
SELECT ROUND(AVG(mileage),2) as average_flight_distance FROM flight;

-- get the average number of seats (all flights).
SELECT ROUND(AVG(a.total_seats),2) AS avg_number_of_seats
FROM flight f
INNER JOIN aircraft a
ON f.aircraft_id = a.id;

-- get the average number of miles flown by customers grouped by status.
SELECT s.status_name as customer_status, ROUND(AVG(f.mileage),2) as miles_flown
FROM customer_status s, 
	customer c, 
	flight f, 
    customer_mileage cm
WHERE f.id = cm.flight_id 
	AND cm.customer_id = c.id
	AND c.status_id = s.id
GROUP BY s.status_name;
	
-- get the maximum number of miles flown by customers grouped by status.
SELECT s.status_name as customer_status, MAX(c.total_customer_mileage) as max_miles_flown
FROM customer_status s, 
	customer c 
...
SELECT COUNT(*) AS number_of_boeing_flights 
FROM flight f
JOIN aircraft a ON f.aircraft_id = a.id
WHERE a.model LIKE "%boeing%";

-- get the total number of aircraft with a name containing Boeing.
SELECT COUNT(*) as number_of_boeing_model FROM aircraft WHERE model LIKE "%boeing%";
...
SELECT s.status_name AS customer_status, ROUND(AVG(f.mileage),2) as flight_distance
FROM flight f
INNER JOIN customer_mileage cm ON f.id = cm.flight_id
INNER JOIN customer c ON c.id = cm.customer_id
INNER JOIN customer_status s ON s.id = c.status_id
GROUP BY s.status_name;

-- find the most often booked aircraft by gold status members (this should require a join).
SELECT MAX(a.model) as most_often_booked_aircraft_by_gold_members
FROM aircraft a
INNER JOIN flight f ON a.id = f.aircraft_id
INNER JOIN customer_mileage cm ON cm.flight_id = f.id
INNER JOIN customer c ON cm.customer_id = c.id
